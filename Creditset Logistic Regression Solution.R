...
creditset<- read.csv("D:/Courses/acadgild/BA with R/Sessions/Session 20/Credit set Logistic regression/creditset.csv")
View(creditset)
dim(creditset)
...
# as default10yr is a categorial 1 being defaulter 0 being non defaulter
...
model<-glm(default10yr~income+age+loan,data=creditset,family= binomial)
model
# Hence our logit function predictors are as below
# Constant term  is 9.9414690
# Coefficient for  Income is  -0.0002434 
#  Coefficient for  age  is  -0.3493025
#  Coefficient for  loan is  0.0017343

# our Logit function becomes  

# logit(odds) = logit(default10yr) =  9.9414690 + ((-0.0002434) * Income) + ((-0.3493025) * age) + ((0.0017343) *loan)
# p(default10yr) = (e^9.9414690 + ((-0.0002434) * Income) + ((-0.3493025) * age) + ((0.0017343) *loan)) / ( 1+ e^9.9414690 + ((-0.0002434) * Income) + ((-0.3493025) * age) + ((0.0017343) *loan))
summary(model)

#trying different models for optimizing 
model1<-glm(default10yr~income,data=creditset,family= binomial)
summary(model1)
model2<-glm(default10yr~age,data=creditset,family= binomial)
summary(model2)
model3<-glm(default10yr~loan,data=creditset,family= binomial)
summary(model3)
model4<-glm(default10yr~age+income,data=creditset,family= binomial)
summary(model4)
model5<-glm(default10yr~age+loan,data=creditset,family= binomial)
summary(model5)
model6<-glm(default10yr~income+loan,data=creditset,family= binomial)
summary(model6)
...
tabtesting0.5<-table(ActualValue=testing$default10yr,PredictedValue=res1>=0.5) #checked with probability cutoff = 0.5
tabtesting0.5
...
# Classification Accuracy for group 0 for training dataset is equal to  "sensitivitytraining0.4" calculated as shown in output

# Classification Accuracy for group 1 is equal to  calculated as "specificitytraining0.4" shown in  output
